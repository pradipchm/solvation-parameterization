...
for i in range(no_of_nodes):
    solute_labels= []
    qb = QueryBuilder()
    eps = load_node(WorkChain_nodes[0].pk).inputs.environ_solution["ENVIRON"]["env_static_permittivity"]
    qb.append(Dict, filters={"attributes.eps": {'==': eps}})
    solvent = qb.all()[0][0].label
    expt_energy_all =  load_node(WorkChain_nodes[0].pk).inputs.expt_energy[:]  ##solutes pks, experimental energies,##
    solute_pks = load_node(WorkChain_nodes[0].pk).inputs.structure_pks[:]
    for pks in solute_pks:
        solute_labels.append(load_node(pks).label)
    
    ### Get all CalcJobNode ids
    
    filtered = filter(lambda x: x.process_class.__name__ == 'EnvPwCalculation', WorkChain_nodes[0].called_descendants)
    ...
    for calc_id in CalcJobNode_ids:
            ...
            if load_node(calc_id).attributes["exit_status"] == 0:
                ...
                solutes[label]['E_vac'] = 0.0
                solutes[label]['E_sol0_0']  = 0.0
                solutes[label]['volume_sol0_0'] = 0.0
                solutes[label]['surface_sol0_0'] = 0.0
                solutes[label]['E_sol1_0'] = 0.0
                solutes[label]['volume_sol1_0'] = 0.0
                solutes[label]['surface_sol1_0'] = 0.0
